...
use enr::{Enr, EnrBuilder, EnrKey, EnrPublicKey};
...
impl Host {
	...
	pub fn new(mut config: NetworkConfiguration, filter: Option<Arc<dyn ConnectionFilter>>) -> Result<Host, Error> {
		...
		let enr = config.config_path.as_ref().and_then(|p| load::<Enr>(Path::new(&p))).filter(|enr| {
			...
			builder.ip(listen_address.ip());
			builder.tcp(listen_address.port());
			builder.udp(udp_port);
			builder.build(&enr_key).expect("ENR always correct; qed")
		});
